...
class LogViewerService {
  ...
  /**
   * Get business logs - only email sending events
          ...
          try {
            ...
            if (logEntry.eventType && businessEvents.includes(logEntry.eventType)) {
              businessLogs.push({
                ...
                // Los datos pueden estar en eventData, metadata o directamente en el log
                emailId: logEntry.eventData?.emailId || logEntry.metadata?.emailId,
                to: logEntry.eventData?.to || logEntry.metadata?.to || logEntry.to,
                from: logEntry.eventData?.from || logEntry.metadata?.from || logEntry.from,
                subject: logEntry.eventData?.subject || logEntry.metadata?.subject || logEntry.subject,
                messageId: logEntry.eventData?.messageId || logEntry.metadata?.messageId,
